...
constexpr uint32_t kRunSaveAnyPayloadWindowMs = 1500U;
...
bool saveRun(const String& payload)
{
    ...
    if (lastRunPayloadMs != 0U && (nowMs - lastRunPayloadMs) <= kRunSaveAnyPayloadWindowMs) {
        return true;
    }
    ...
    filter["meta"]["reason"] = true;
    filter["meta"]["xAxis"] = true;
    filter["meta"]["mode"] = true;
    filter["meta"]["session"] = true;
    filter["meta"]["samples"] = true;
    filter["peaks"]["hp"] = true;
    filter["peaks"]["hp_rpm"] = true;
    filter["peaks"]["torque"] = true;
    filter["peaks"]["torque_rpm"] = true;
    filter["peaks"]["egt"] = true;
    filter["points"][0]["r"] = true;
    filter["points"][0]["x"] = true;
    filter["points"][0]["h"] = true;
    filter["points"][0]["t"] = true;
    filter["points"][0]["b"] = true;
    filter["points"][0]["m"] = true;
    filter["points"][0]["l"] = true;

    JsonDocument doc;
    if (deserializeJson(doc, payload, DeserializationOption::Filter(filter)) != DeserializationError::Ok) return false;
    ...
    stored["save_reason"] = meta["reason"] | "";
    stored["x_axis"] = meta["xAxis"] | "";
    stored["run_mode"] = meta["mode"] | "";
    stored["session"] = meta["session"] | 0;
    stored["samples"] = meta["samples"] | 0;
